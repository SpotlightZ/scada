...
const prop = defineProps<{ activeItem: ToolItem }>();

type ToolItem = 'circle' | 'rect' | 'cursor' | 'line' | string;

const emit = defineEmits(['change']);
const selected = ref('circle');

const handleItemClick = (name: ToolItem) => {
    selected.value = name;
    emit('change', name);
};

const options: { name: ToolItem, icon: string }[] = [
    {
        name: 'cursor',
...
<template>
    <div class="rounded-lg p-1.5 flex flex-row gap-1 bg-white shadow-lg border-solid border z-[910000]">
        <button v-for="option in options" :key="option.name" @click="handleItemClick(option.name)"
            :class="[selected === option.name ? 'button-active' : 'button-normal']" v-html="option.icon">
        </button>
    </div>
</template>
