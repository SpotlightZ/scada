...
import { ElementListPanel } from '../core/panel/ElementListPanel'

const panel = new ElementListPanel(() => {
    panel.getAllElementMeta().forEach((entry) => {
        options.push({
            id: entry.id,
            name: entry.label,
            icon: entry.icon
...
<template>
    <div class="rounded-lg p-1.5 flex flex-row gap-1 bg-white shadow-lg border-solid border z-[910000]">
        <button v-for="option in options" :key="option.id" @click="handleItemClick(option.id)"
            :class="[selected === option.id ? 'button-active' : 'button-normal']" v-html="option.icon">
        </button>
    </div>
</template>
...
<style scoped>
.button-normal {
    @apply hover:bg-blue-100 text-white py-2.5 px-2.5 rounded-lg;
}


.button-active {
    @apply bg-blue-200 text-white py-2.5 px-2.5 rounded-lg;
}
</style>
